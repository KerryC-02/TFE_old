    ...
    SubShader
    {
        ...
        Pass
        {
            ...
            float3 Layer(float2 UV, float t) {
                ...
                float2 uv = UV * _Size * aspect;//Size密度
                uv.y += t * .25;
                float2 gv = frac(uv) - .5;
                float2 id = floor(uv);
                ...
                float w = UV.y * 10;
                float x = (n - .5) * .8;//x(-.4 .4)
                x += (.4 - abs(x)) * sin(3 * w) * pow(sin(w), 6) * .45;//x轴的水滴正弦曲线移动

                float y = -sin(t + sin(t + sin(t) * .5)) * .45;//y轴的水滴正弦曲线移动
                y -= (gv.x - x) * (gv.x - x);//圆形水滴或椭圆水滴

                float2 dropPos = (gv - float2(x, y)) / aspect;
                float drop = S(.05, .03, length(dropPos));//水滴大小
                ...
                fogTrail *= S(.5, y, gv.y);
                trail *= fogTrail;
                fogTrail *= S(.05, .04, abs(dropPos.x));//水滴踪迹
            ...
            fixed4 frag(v2f i) : SV_Target
            {
                ...
                //col = tex2Dlod(_MainTex, float4 (i.uv+drops.xy*_Distortion,0,blur));//毛玻璃效果（设置图片是三线性）

                float2 projUv = i.grabUv.xy / i.grabUv.w;
                projUv += drops.xy * _Distortion * fade;
                blur *= .01;

                const float numSamples = 32;//采样数
